...
class BNCall:
    ...
    def get_price(self, symbols, token):
        global highest_perc
        global is_first_send_bnb
        global is_first_send_btc
        ...
        if symbols == 'BNBUSDT' and is_first_send_bnb is False:
            is_first_send_bnb = True
            msg = '%s-%s\t%.4f\t%s%%' % (time_str, symbols, price_current, percent_diff_txt)
            self.send_message_to_bot(token, msg)
        if symbols == 'BTCUSDT' and is_first_send_btc is False:
            is_first_send_btc = True
            msg = '%s-%s\t%.4f\t%s%%' % (time_str, symbols, price_current, percent_diff_txt)
            self.send_message_to_bot(token, msg)
